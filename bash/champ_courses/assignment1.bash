...
function displayClassroomCourses(){

echo -n "Please Input a Classroom: "
read classroomName

echo ""
echo "Courses in $classroomName :"
cat "$courseFile" | grep "$classroomName" | cut -d ';' -f1,2,5,6,7 | \
sed 's/;/ | /g'
echo ""
...
cat "$courseFile" | grep "$subName" | awk -F';' '$4>0' | \
sed 's/;/ | /g'
echo ""
